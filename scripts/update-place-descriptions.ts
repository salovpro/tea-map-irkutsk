 ...
 * Matches by unique slug only. Does not change names, addresses, coordinates,
 * tea_menu, EN/ZH, sortOrder, or create/delete rows.
 *
 ...
 * Existing `Адрес: …` suffix is preserved so catalog addresses are not dropped.
 */
import "dotenv/config";
import { PrismaPg } from "@prisma/adapter-pg";
import { Locale, PrismaClient } from "../src/generated/prisma/client";
import { extractDescriptionBody } from "../src/lib/place-description";
...
const updates: DescriptionUpdate[] = [
  ...
  },
  {
    ...
    slug: "chempiony",
    description:
      "Это не просто бар или ресторан, а ещё и музей Иркутского спорта с мужской игровой комнатой и караоке. Море еды и трансляции спорственных матчей.",
  },
  {
...
function splitRuAddressSuffix(description: string): {
  body: string;
  addressLine: string | null;
} {
  const match = description.match(/(\n\s*)?(Адрес:\s*[\s\S]*)$/i);
  if (!match || match.index == null) {
    return { body: extractDescriptionBody(description), addressLine: null };
  }
  return {
...
function composeDescription(existing: string, approvedBody: string): string {
  const { addressLine } = splitRuAddressSuffix(existing);
  const body = approvedBody.trim();
  if (!addressLine) return body;
  if (
    body.includes(addressLine) ||
    body.includes(addressLine.replace(/\.$/, ""))
  ) {
    return body;
  }
  return `${body}\n\n${addressLine}`;
...
async function main() {
  ...
  for (const row of matched) {
    console.log(`- ${row.requestedName}`);
    console.log(`  id: ${row.id}`);
    console.log(`  slug: ${row.slug}`);
    console.log(`  RU title: ${row.ruTitle}`);
    console.log("  OLD:");
  ...
  console.log("");
  console.log("MISSING_RU_TRANSLATION:");
  ...
  if (!apply) {
    ...
    if (!ready) {
      console.log("Abort: unmatched, ambiguous, or missing RU translations.");
    }
    return;
  }
  ...
  for (const row of matched) {
    const place = readBySlug.get(row.slug);
    const description = place?.translations[0]?.description ?? "";
    if (description !== row.newDescription) failed += 1;
    console.log(`- ${row.slug} / ${row.id}`);
  }

  console.log("");
  console.log(`UPDATED: ${matched.length - failed}`);
